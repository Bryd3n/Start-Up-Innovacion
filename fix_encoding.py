...
def fix_file(filepath):
    ...
    # 3. Replace old UniNotes logo format
    content_str = re.sub(r'Uni<span style="color:\s*var\(--primary\)">Notes</span>', '<span class="text-gradient">UniNotes</span>', content_str)
    
    # 4. Convert Menu QR Express to UniNotes
    content_str = re.sub(r'Menu QR Express', 'UniNotes', content_str)
    
    # 5. Wrap all unwrapped UniNotes
    content_str = re.sub(r'(?<!<span class="text-gradient">)UniNotes(?!</span>)', '<span class="text-gradient">UniNotes</span>', content_str)
    
    # 6. Fix the title tag which should not have spans
